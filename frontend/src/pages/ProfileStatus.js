...
export default function ProfileStatus() {
  ...
  const handlePayFees = () => setShowPayment(true);
  const handlePaymentChange = e => setPaymentDetails(d => ({ ...d, [e.target.name]: e.target.value }));
  const handlePayNow = async e => {
    e.preventDefault();
    setPayError("");
    // Validate: only numbers allowed
    if (!/^[0-9]+$/.test(paymentDetails.card) || !/^[0-9]+$/.test(paymentDetails.amount)) {
      setPayError("Only numbers are allowed in Card Number and Amount.");
      return;
    }
    setPaying(true);
    const token = localStorage.getItem('token');
        ...
        >
          <div className="relative group">
            <div className="relative bg-black/40 backdrop-blur-xl rounded-2xl p-6 border border-white/[0.05] shadow-2xl overflow-hidden">
              ...
              {showPayment && (
                <form onSubmit={handlePayNow} className="pay-fees-form mt-6 flex flex-col gap-6">
                  {payError && (
